...
const Inspector = () => {
    ...
    if (selection.type === 'TEMPO_POINT') {
        ...
        // We need to track the "current" scale relative to the moment selection started (100%).
        // But since we modify the ACTUAL values in real-time, "100%" always represents the CURRENT state if we don't track original.
        // Wait, if we drag 100 -> 50, values compress.
        // If we then drag 50 -> 100, we want them to expand back to original?
        // YES.
        // However, standard Draggable Input modifies the value incrementally.
        // If we use the "Ratio" approach (New / Old), we can just modify the current values on the fly.
        // 100 -> 50 (Ratio 0.5). values * 0.5 variance.
        // 50 -> 100 (Ratio 2.0). values * 2.0 variance.
        // This works stateless regarding "Original" values, as long as we track the "Previous Scale" to calculate Ratio.
        ...
        useEffect(() => {
            setScalePct(100);
        }, [selection.ids]); // dependency on IDs stringified? ref identity should be ok if immutable.

        if (count === 0) return <div style={containerStyle}>No Selection</div>;

        // Get points
        const points = tempoSettings.points.filter(p => selectedIds.includes(p.id));
